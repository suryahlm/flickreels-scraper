#!/usr/bin/env python3
"""
R2 Cleanup Script - Delete non-Indonesian drama folders
Keeps only the 52 strictly Indonesian dramas
"""
import boto3
...
# 52 strictly Indonesian drama IDs to KEEP
KEEP_IDS = {
...
def main():
    print("=" * 60)
    print("R2 CLEANUP - DELETE NON-INDONESIAN DRAMAS")
    print("=" * 60)
    ...
    print("\n--- FOLDERS TO DELETE ---")
    for i, (folder, name) in enumerate(to_delete[:10]):
        print(f"  {i+1}. {name}")
    if len(to_delete) > 10:
        print(f"  ... and {len(to_delete)-10} more")
    
    # Confirm
    print(f"\n⚠️  This will delete {len(to_delete)} folders and free up ~100GB storage!")
