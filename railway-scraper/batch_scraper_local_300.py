...
class IndonesianBatchScraper:
    ...
    def discover_and_scrape(self, limit=None, fresh=False):
        ...
        logger.info("\n" + "=" * 60)
        logger.info("INDONESIAN BATCH SCRAPER - Local Edition")
        logger.info("Using nav_id=30 for Indonesian content")
        logger.info("=" * 60 + "\n")
        
        # Discover Indonesian dramas from nav_id=30
        # Note: API pagination is broken, so we just fetch nav_id=30 once
        all_dramas = {}
        logger.info("Discovering Indonesian dramas (nav_id=30)...")
        dramas = self.api.get_indonesian_dramas(page=1, nav_id="30")
        
        for d in dramas:
            if d["id"] not in all_dramas:
                all_dramas[d["id"]] = d
        
        logger.info(f"Found {len(all_dramas)} Indonesian dramas")
