#!/usr/bin/env python3
"""
FlickReels Indonesian Batch Scraper - LOKAL SCRAPING Edition
=============================================================
Script untuk scraping drama Indonesia dari PC lokal.

PERBEDAAN dengan batch_scraper_indonesia.py:
1. Tambahan ID scanning untuk menemukan drama Indonesia baru
2. Cek language_name == "indonesian" untuk filter asli
3. Multiple discovery sources (navigation + ID scan)

Features:
- Skip drama yang sudah ada di Supabase
- Cover dari API (bukan segment)
- Metadata lengkap
- Auto-publish ke Supabase
- 12 parallel episode downloads

Usage:
    python local_scraping_indonesia.py --limit=3    # Test dengan 3 drama
    python local_scraping_indonesia.py --limit=10   # Scrape 10 drama baru
"""
import os
import sys
import re
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('local_scraping.log', encoding='utf-8')
    ]
)
...
# ============================================================================
# API CLIENT - Enhanced with ID Scanning
# ============================================================================
...
class IndonesianAPI:
    ...
    def get_latest_dramas(self, page=1, page_size=50):
        """Get dramas from latestPlay endpoint"""
    ...
    def get_drama_detail(self, drama_id):
        """Get drama detail with episodes AND language check"""
        ...
        return {
            ...
            "language_name": data.get("language_name", ""),  # IMPORTANT for filtering!
            "episodes": [
                {
...
# ============================================================================
# LOCAL SCRAPER - Enhanced Discovery
# ============================================================================

class LocalIndonesianScraper:
    """Lokal Scraping - Enhanced with ID scanning for Indonesian dramas"""
    ...
    def _load_progress(self):
        """Load already scraped IDs from Supabase (skip existing)"""
        try:
            resp = requests.get(
    ...
    def _save_progress(self, drama_id):
        ...
        with open("local_scraped_ids.txt", "a") as f:
            f.write(f"{drama_id}\n")
    ...
    def scrape_episode(self, drama_id, episode_num, chapter_id, r2_base):
        ...
        try:
            ...
            logger.info(f"  [EP {episode_num}] Done - {len(segments)} segments")
            return True
    ...
    def scrape_drama(self, drama):
        """Scrape single drama with 12 concurrent episodes. Returns True if successful."""
        drama_id = drama["id"]
        title = drama["title"]
        ...
        # Download and upload cover image FROM API (not segment!)
        cover_url = drama.get("cover") or detail.get("cover", "")
        r2_cover_path = ""
        if cover_url:
            logger.info(f"Downloading cover image from API...")
            try:
    ...
    def discover_indonesian_dramas(self, limit=None):
        """
        ENHANCED DISCOVERY - Multiple sources + ID scanning
        Returns only REAL Indonesian dramas (language_name == "indonesian")
        """
        collected = {}  # id -> drama info
        
        # =====================================================================
        # SOURCE 1: Navigation Column (nav_id=30)
        # =====================================================================
        logger.info("Source 1: Fetching from navigationColumn...")
        page = 1
        empty_count = 0
        while empty_count < 3:
            ...
            if not dramas:
                ...
                empty_count = 0
                for d in dramas:
                    if d["id"] not in self.scraped_ids and d["id"] not in collected:
                        collected[d["id"]] = d
            page += 1
            if limit and len(collected) >= limit:
                break
        logger.info(f"  Found {len(collected)} from navigationColumn")
        
        # =====================================================================
        # SOURCE 2: Latest Play (paginated)
        # =====================================================================
        if not limit or len(collected) < limit:
            logger.info("Source 2: Fetching from latestPlay...")
            for page in range(1, 20):
                dramas = self.api.get_latest_dramas(page=page, page_size=50)
                if not dramas:
                    break
                for d in dramas:
                    drama_id = str(d.get("playlet_id"))
                    if drama_id not in self.scraped_ids and drama_id not in collected:
                        collected[drama_id] = {
                            "id": drama_id,
                            "title": d.get("title", ""),
                            "cover": d.get("cover", ""),
                            "total_episodes": int(d.get("upload_num") or 0),
                            "description": d.get("introduce", ""),
                            "tags": d.get("playlet_tag_name", [])
                        }
                if limit and len(collected) >= limit:
                    break
                time.sleep(0.3)
        ...
        # =====================================================================
        if not limit or len(collected) < limit:
            logger.info("Source 3: Fetching from hotRank...")
        ...
        # =====================================================================
        # SOURCE 4: ID Scanning (find Indonesian dramas by language check)
        # =====================================================================
        if not limit or len(collected) < limit:
            logger.info("Source 4: Scanning ID range for Indonesian dramas...")
            scan_count = 0
            for scan_id in range(6000, 1000, -1):  # Start from recent
                if limit and len(collected) >= limit:
                    break
                
                scan_id_str = str(scan_id)
                if scan_id_str in self.scraped_ids or scan_id_str in collected:
                    continue
                
                # Get drama details and check language
                detail = self.api.get_drama_detail(scan_id_str)
                if detail:
                    title = detail.get("title", "")
                    language = detail.get("language_name", "").lower()
                    episodes = detail.get("episodes", [])
                    
                    # ONLY include if language is INDONESIAN
                    if title and len(episodes) > 0 and "indonesian" in language:
                        collected[scan_id_str] = {
                            "id": scan_id_str,
                            "title": title,
                        ...
                        scan_count += 1
                        logger.info(f"  Found ID {scan_id_str}: {title} ({len(episodes)} eps)")
                        
                        if limit and len(collected) >= limit:
                            break
                
                # Limit scanning to avoid too many API calls
                if scan_count >= 10:
                    break
            
            logger.info(f"  Found {scan_count} via ID scanning")
        
        return list(collected.values())
    
    def run(self, limit=None):
        """Main entry point - keeps scraping until target is reached"""
        logger.info("\n" + "=" * 60)
        logger.info("LOKAL SCRAPING - Indonesian Drama Scraper")
        logger.info("Enhanced with ID scanning for real Indonesian content")
        logger.info("=" * 60 + "\n")
        
        target_count = limit or 999  # If no limit, scrape all found
        success_count = 0
        attempt_count = 0
        max_attempts = target_count * 3  # Avoid infinite loop
        
        # For large targets, discover in batches
        batch_size = min(50, target_count)  # Discover 50 at a time
        
        # Initial discovery
        all_dramas = self.discover_indonesian_dramas(limit=batch_size)
        dramas_queue = [d for d in all_dramas if str(d["id"]) not in self.scraped_ids]
        
        logger.info(f"\nFound {len(dramas_queue)} initial candidates, targeting {target_count} successful scrapes\n")
        
        if not dramas_queue:
            logger.info("No new dramas to scrape!")
            return
        
        # Keep scraping until target is reached
        drama_index = 0
        while success_count < target_count and attempt_count < max_attempts:
            # Re-discover if running low on candidates
            if drama_index >= len(dramas_queue):
                logger.info(f"\n📡 Re-discovering more Indonesian dramas...")
                new_dramas = self.discover_indonesian_dramas(limit=batch_size)
                new_queue = [d for d in new_dramas if str(d["id"]) not in self.scraped_ids]
                if not new_queue:
                    logger.warning("No more new dramas available!")
                    break
                dramas_queue.extend(new_queue)
                logger.info(f"Added {len(new_queue)} more candidates. Queue: {len(dramas_queue) - drama_index} remaining")
            
            drama = dramas_queue[drama_index]
            drama_index += 1
            attempt_count += 1
            
            logger.info(f"\n[{success_count+1}/{target_count}] Starting {drama['title']} (ID: {drama['id']})...")
            
            # Try to scrape - returns True if successful
            if self.scrape_drama(drama):
                success_count += 1
                logger.info(f"✅ SUCCESS ({success_count}/{target_count})")
            else:
                logger.warning(f"⚠️ SKIPPED/FAILED - will try next drama")
        
        # Final report
        elapsed = time.time() - self.stats["start_time"]
        logger.info(f"\n{'='*60}")
        logger.info("LOKAL SCRAPING COMPLETE!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Lokal Scraping - Indonesian Drama Scraper")
